...
if (Test-Connection "raw.githubusercontent.com" -Count 1 -Quiet) {
  ...
  $latest_version = (Invoke-WebRequest -Uri "https://raw.githubusercontent.com/bygalacos/Zabbix_Mass_Import/main/Windows/6.2/version.txt").Content
  ...
  if ($current_version -ne $latest_version) {
    ...
    $changelog = (Invoke-WebRequest -Uri "https://raw.githubusercontent.com/bygalacos/Zabbix_Mass_Import/main/Windows/6.2/changelog.txt").Content
    ...
    if ($update_confirm -eq "y") {
      ...
      Invoke-WebRequest -Uri "https://raw.githubusercontent.com/bygalacos/Zabbix_Mass_Import/main/Windows/6.2/zabbix_mass_import_6.2.ps1" -OutFile $PSCommandPath
...
Add-Content -Path $f_output ([char]9 + "<version>6.2</version>")
